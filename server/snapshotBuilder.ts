      ...
      result?: Array<{
        values?: Array<
          Record<
            string,
            {
              open?: Array<{ ts?: string | number; v: number }>;
              high?: Array<{ v: number }>;
              low?: Array<{ v: number }>;
...
export async function buildBasketSnapshot(
  ...
  const legFetches = legs.map((leg) => {
    ...
    return fetchHistorical(fetchTs, leg.zanskarName, type, startDate, endDate).then((bars) => ({
      leg,
      bars,
    }));
  });
  ...
  for (const { leg, bars } of legResults) {
    ...
    legPriceData.set(
      leg.refId,
      sessionBars.map((b) => ({ time: b.time, value: b.close })),
    );
  ...
  const totalPnl =
    basketPnlData.length > 0 ? basketPnlData[basketPnlData.length - 1].value * 100 : 0; // → paise
  ...
  return {
    underlying,
    tradeDate: istDateString(earliestNs),
    data,
    totalPnl,
