...
describe('findCases', () => {
  ...
  test('kept cases start and end at least `spacingMinutes` apart, up to the per-day cap', () => {
    const day = makeDay(
      () => 23250,
    ...
    for (let a = 0; a < spans.length; a++) {
      for (let b = a + 1; b < spans.length; b++) {
        expect(Math.abs(spans[a][0] - spans[b][0])).toBeGreaterThanOrEqual(25);
        expect(Math.abs(spans[a][1] - spans[b][1])).toBeGreaterThanOrEqual(25);
      }
    }
    // Strongest first.
    const scores = scan.cases.map((c) => Math.abs(c.totalDelta));
