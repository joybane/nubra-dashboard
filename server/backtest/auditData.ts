...
function tradingDays(a: string, b: string): string[] {
  const out: string[] = [];
  const s = new Date(`${a}T12:00:00Z`),
    e = new Date(`${b}T12:00:00Z`);
...
function dte(date: string, expiry: string): number {
  return Math.round(
    (Date.parse(`${expiry}T00:00:00Z`) - Date.parse(`${date}T00:00:00Z`)) / 86400000,
  );
}
...
async function run() {
  ...
  // print table
  const hdr =
    'date        expiry      dte  atm     spotCol   medianF   diff     F-spread  strikes  flag';
  console.log(hdr);
  ...
  console.log(
    `flagged misaligned  : ${flagged.length}  (${rows.length ? Math.round((flagged.length / rows.length) * 100) : 0}%)`,
  );
  console.log(
    `  of which premiums agree across strikes (F-spread<50) → spot column is the outlier: ${tightSpread.length}`,
  );
  console.log(
    `avg |diff| flagged  : ${flagged.length ? Math.round(flagged.reduce((a, r) => a + Math.abs(r.diff), 0) / flagged.length) : 0} pts`,
  );
  console.log(
    `median diff (all)   : ${Math.round(median(rows.map((r) => r.diff)))} pts  (small + = normal carry)`,
  );
