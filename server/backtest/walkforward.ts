...
export async function runWalkForward(req: WalkForwardRequest): Promise<WalkForwardResponse> {
  ...
  if (span < windows * 7) {
    return {
      ...
      efficiency: 0,
      equityCurve: [],
      error: 'Date range too short for the requested number of windows.',
    };
  }
  ...
  for (let w = 0; w < windows; w++) {
    ...
    for (const v of values) {
      ...
      if (val > bestMetric) {
        bestMetric = val;
        bestParam = v;
        bestDailyMean = trades.length ? m.totalPnl / trades.length : 0;
      }
    }
    ...
    for (const t of oosTrades) {
      cum += t.pnl;
      equityCurve.push({ date: t.date, cumPnl: Math.round(cum * 100) / 100 });
    }
    ...
    results.push({
      ...
      oosTo,
      bestParam,
      isMetric: Math.round(bestMetric * 100) / 100,
      oosPnl: Math.round(oosM.totalPnl * 100) / 100,
  ...
  const efficiency =
    isImpliedTotal !== 0 ? Math.round((oosTotalPnl / isImpliedTotal) * 1000) / 1000 : 0;
