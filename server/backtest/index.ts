...
export function validateConfig(cfg: BacktestConfig): string | null {
  ...
  for (const l of cfg.legs) {
    ...
    if (l.reentry && l.reentry.mode !== 'NONE') {
      if (!Number.isFinite(l.reentry.max ?? 0) || (l.reentry.max ?? 0) < 0)
        return `Leg ${l.id}: re-entry max must be ≥ 0.`;
    }
    if (l.trail && l.trail.type !== 'NONE') {
      if (!Number.isFinite(l.trail.trigger ?? 0) || (l.trail.trigger ?? 0) < 0)
        return `Leg ${l.id}: trail trigger must be ≥ 0.`;
      if ((l.trail.type === 'TRAIL' || l.trail.type === 'LOCK_AND_TRAIL') && !(l.trail.step! > 0))
        return `Leg ${l.id}: trail step must be > 0.`;
    }
  }
  const f = cfg.entryFilters;
  if (f) {
    if (f.dteMin != null && f.dteMax != null && f.dteMin > f.dteMax)
      return 'entryFilters: dteMin must be ≤ dteMax.';
    if (f.ivMin != null && f.ivMax != null && f.ivMin > f.ivMax)
      return 'entryFilters: ivMin must be ≤ ivMax.';
    if (f.premiumMin != null && f.premiumMax != null && f.premiumMin > f.premiumMax)
      return 'entryFilters: premiumMin must be ≤ premiumMax.';
    if (f.waitTradePct != null && f.waitTradePct < 0)
      return 'entryFilters: waitTradePct must be ≥ 0.';
  }
  return null;
}
...
export async function runDayDetail(cfg: BacktestConfig, date: string): Promise<DayDetailResponse> {
  ...
  if (!res)
    return {
      ok: false,
      error: 'No trade for that date (holiday / no data / entry filter not met).',
    };
...
export function validateSweep(req: SweepRequest): string | null {
  ...
  if (!pathIsValid(req.base, req.param1.path))
    return `param1.path "${req.param1.path}" does not resolve to a settable field (check the leg index and that its SL/target value exists).`;
  if (!Number.isFinite(req.param1.from) || !Number.isFinite(req.param1.to))
    return 'param1 from/to required.';
  ...
  if (req.param2) {
    ...
    if (!pathIsValid(req.base, req.param2.path))
      return `param2.path "${req.param2.path}" does not resolve to a settable field (check the leg index and that its SL/target value exists).`;
    if (!Number.isFinite(req.param2.from) || !Number.isFinite(req.param2.to))
      return 'param2 from/to required.';
    if (!Number.isFinite(req.param2.step) || req.param2.step <= 0)
      return 'param2.step must be > 0.';
    const steps2 = Math.abs(req.param2.to - req.param2.from) / req.param2.step + 1;
    if (steps1 * steps2 > 500)
      return `Sweep grid too large (${Math.round(steps1)}×${Math.round(steps2)}=${Math.round(steps1 * steps2)}). Max 500 cells.`;
  } else {
...
export function validateWalkForward(req: WalkForwardRequest): string | null {
  ...
  if (!pathIsValid(req.base, req.param.path))
    return `param.path "${req.param.path}" does not resolve to a settable field (check the leg index and that its SL/target value exists).`;
  if (!Number.isFinite(req.param.step) || req.param.step <= 0) return 'param.step must be > 0.';
  if (!Number.isFinite(req.windows) || req.windows < 2 || req.windows > 12)
    return 'windows must be between 2 and 12.';
  if (!Number.isFinite(req.oosPct) || req.oosPct < 5 || req.oosPct > 80)
    return 'oosPct must be between 5 and 80.';
  const steps = Math.abs(req.param.to - req.param.from) / req.param.step + 1;
  if (steps * req.windows > 400)
    return `Walk-forward too large (${Math.round(steps)} steps × ${req.windows} windows). Reduce range or windows.`;
  return null;
}
