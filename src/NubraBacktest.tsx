...
let _idSeq = 0;
function genId() {
  return `nbl_${Date.now().toString(36)}_${++_idSeq}`;
}
...
export default function NubraBacktest({ instrument, theme = 'dark' }: Props) {
  ...
  const getGreeksAtTime = useCallback(
    (t: number) => {
      ...
      for (let legIdx = 0; legIdx < legs.length; legIdx++) {
        ...
        // Spot as the forward proxy — no CE/PE pair here to imply one, and spot measures much
        // closer than spot·e^{rT} for NIFTY (negative basis). See forwardFromParity.
        let iv = impliedVolatility(ltp, spotPrice, K, T, RISK_FREE, type);
        if (iv <= 0 || !isFinite(iv)) iv = 0.2;

        const g = blackScholes(spotPrice, K, T, RISK_FREE, iv, type);
  ...
  function updateLeg(id: string, u: Partial<Leg>) {
  ...
  useEffect(() => {
    ...
    const updateAllTooltips = (
      ...
      if (priceContainerRef.current && priceTooltipRef.current) {
        ...
        priceTooltipRef.current.setPosition(
          alignLeft ? xAdj - 20 : xAdj + 25,
          currentActiveChart === 'price'
            ? Math.max(8, Math.min((activeChartY ?? defaultY) - 80, h - 100))
            : 8,
          alignLeft,
        );
      ...
      if (pnlContainerRef.current && pnlTooltipRef.current) {
        ...
        const alignLeft = xAdj > w * 0.5;
        pnlTooltipRef.current.setPosition(
          alignLeft ? xAdj - 20 : xAdj + 25,
          currentActiveChart === 'pnl'
            ? Math.max(8, Math.min((activeChartY ?? defaultY) - 80, h - 100))
            : 8,
          alignLeft,
        );
        ...
        if (param.point && param.time != null) {
          ...
          if (res) {
            if (pnlChart && basketSeries)
              pnlChart.setCrosshairPosition(res.totalPnl, param.time, basketSeries);
            if (greeksChart) {
    ...
    if (pnlChart && basketSeries) {
      pnlChart.subscribeCrosshairMove((param) => {
        if (param.point && param.time != null) {
          ...
          if (res) {
            if (priceChart && indexSeries)
              priceChart.setCrosshairPosition(res.spot ?? 0, param.time, indexSeries);
            if (greeksChart) {
              const src = activeGreekSource(greeksLegFilter);
              const activeS = greeksSeriesMap[`${src}_delta`];
              if (activeS) greeksChart.setCrosshairPosition(res.delta, param.time, activeS);
  ...
  const totalPremium = useMemo(
    () => legs.reduce((acc, l) => acc + (l.side === 'BUY' ? -1 : 1) * l.ltp * l.lots * lotSize, 0),
    [legs, lotSize],
  );
  ...
  const onDrag = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) =>
        setLeftWidth(Math.max(320, Math.min(800, startW + (ev.clientX - startX))));
      const onUp = () => {
        setIsDragging(false);
          ...
          {['NIFTY', 'SENSEX'].map((u) => (
            <button
              ...
              style={{
                ...
                border: 'none',
                cursor: 'pointer',
                background: underlying === u ? '#5865f2' : 'transparent',
                color: underlying === u ? '#fff' : 'var(--text-secondary)',
              }}
            >
              {u}
            </button>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            >
              {availableExpiries.map((ae) => (
                <option key={ae.expiry} value={ae.expiry}>
                  {formatExpiry(ae.expiry)} ({ae.flag})
                </option>
              ))}
            </select>
          </label>
                  ...
                  <div className="px-3 py-2 flex flex-wrap gap-1.5">
                    ...
                    >
                      <span
                        ...
                        style={{
                          backgroundColor: showSpotPrice ? '#3b82f6' : 'transparent',
                  ...
                  <div className="px-3 py-2 flex flex-wrap gap-1.5">
                    ...
                    >
                      <span
                        className="w-1.5 h-1.5 rounded-full"
                        style={{
                          backgroundColor: showTotalPnl ? '#ffffff' : 'transparent',
                ...
                (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div
                          className="flex items-center bg-[var(--bg-primary)] rounded overflow-hidden border"
                          style={{ borderColor: 'var(--border)' }}
                        >
                          <button
                          ...
                          <div className="flex items-center gap-1 text-[10px] text-[var(--text-muted)]">
                            ...
                            {editingLotSize ? (
                              ...
                              <button
                                ...
                                className="text-[var(--text-primary)] hover:text-[#a78bfa] transition-colors underline decoration-dotted font-semibold"
                              >
                                {lotSize}
                              </button>
                            )}
                            ...
                            return (
                              <button
                                ...
                                style={
                                  active
                                    ? {
                                        backgroundColor: gColors[k] + '33',
                                        color: gColors[k],
                                        borderColor: gColors[k] + '55',
                                        cursor: 'pointer',
                                      }
                                    : { cursor: 'pointer' }
                                }
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px]">
                          {activeGreeks.map((k) => (
                            <div key={k} className="flex items-center gap-1">
                              ...
                              <span
                                className={`font-semibold tabular-nums ${(netGreeks as any)[k] >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                              >
                                {(netGreeks as any)[k] >= 0 ? '+' : ''}
                                {fmtG((netGreeks as any)[k], k)}
                              </span>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="px-3 py-1.5">
                        <button
                          ...
                          style={{ background: 'none', border: 'none', cursor: 'pointer' }}
                        >
                          <span className="text-[8px]">{greeksExpanded ? '▾' : '▸'}</span> Leg
                          breakdown
                        </button>
                        {greeksExpanded && (
                          <div
                            ...
                            style={{
                              gridTemplateColumns: `20px 1fr ${activeGreeks.map(() => '48px').join(' ')} 40px`,
                            }}
                          >
          ...
          {evalResult && (
            <span
              style={{
                fontSize: 12,
                fontWeight: 600,
                color: evalResult.grossPnl >= 0 ? 'var(--green)' : 'var(--red)',
              }}
            >
              ...
              {legs.length === 0 ? (
                ...
                <>
                  <div
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '30px 58px 60px 30px 68px 52px 24px',
                      alignItems: 'center',
                      gap: 2,
                      padding: '4px 0',
                      borderBottom: '1px solid var(--border)',
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            fontSize: 10,
                            background:
                              leg.optionType === 'CE' ? 'var(--green-dim)' : 'var(--red-dim)',
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          ...
                          <span
                            style={{ fontSize: 11, fontWeight: 600, flex: 1, textAlign: 'center' }}
                          >
                            {leg.lots * lotSize}
                          </span>
                          <button
                        ...
                        <span
                          style={{ fontSize: 10, color: prem >= 0 ? 'var(--green)' : 'var(--red)' }}
                        >
                          {inr(prem)}
                        </span>
                        <button
                  ...
                  <button
                    onClick={simulate}
                    disabled={evalLoading || !legs.length}
                    style={{
                      width: '100%',
                      padding: '10px 0',
                      borderRadius: 8,
                      ...
                      fontSize: 13,
                      cursor: 'pointer',
                      opacity: evalLoading || !legs.length ? 0.5 : 1,
                      marginTop: 6,
                    }}
                  >
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '0 4px' }}>
              ...
              {!chainLoading && !chainError && chain.length > 0 && (
                <table style={{ width: '100%', fontSize: 11, borderCollapse: 'collapse' }}>
                  ...
                  >
                    <tr>
                      {['OI', 'Vol', 'IV', 'CE LTP', 'Strike', 'PE LTP', 'IV', 'Vol', 'OI'].map(
                        (h) => (
                          <th
                            key={h}
                            style={{
                    ...
                    {chain.map((row) => {
                      ...
                      return (
                        <tr
                          ...
                          style={{
                            borderBottom: '1px solid var(--bg-card)',
                            background: isAtm ? 'rgba(88,101,242,0.08)' : undefined,
                          }}
                        >
                          <td
                            style={{
                          ...
                          >
                            {row.ceOi ? (row.ceOi / 1000).toFixed(0) + 'K' : '—'}
                          </td>
                          <td
                            style={{
                              padding: '4px 3px',
                          ...
                          >
                            {row.ceVol ? (row.ceVol / 1000).toFixed(0) + 'K' : '—'}
                          </td>
                          <td
                            style={{
                              padding: '4px 3px',
                          ...
                          >
                            {row.ceIv > 0 ? (row.ceIv * 100).toFixed(2) + '%' : '—'}
                          </td>
                          <td
                            style={{ padding: '4px 3px', textAlign: 'right', cursor: 'pointer' }}
                            onClick={() => addLeg(row.strike, 'CE', 'SELL')}
                            onContextMenu={(e) => {
                              e.preventDefault();
                              addLeg(row.strike, 'CE', 'BUY');
                            }}
                          ...
                          <td
                            style={{
                              ...
                              fontWeight: 700,
                              fontSize: 11,
                              color: isAtm ? '#5865f2' : 'var(--text-primary)',
                              background: isAtm ? 'rgba(88,101,242,0.05)' : undefined,
                            }}
                          >
                            {row.strike.toLocaleString('en-IN')}
                          </td>
                          <td
                            style={{ padding: '4px 3px', textAlign: 'left', cursor: 'pointer' }}
                            onClick={() => addLeg(row.strike, 'PE', 'SELL')}
                            onContextMenu={(e) => {
                              e.preventDefault();
                              addLeg(row.strike, 'PE', 'BUY');
                            }}
                            title="Left-click: SELL  |  Right-click: BUY"
                          >
                          ...
                          >
                            {row.peIv > 0 ? (row.peIv * 100).toFixed(2) + '%' : '—'}
                          </td>
                          <td
                            style={{
                              padding: '4px 3px',
                          ...
                          >
                            {row.peVol ? (row.peVol / 1000).toFixed(0) + 'K' : '—'}
                          </td>
                          <td
                            style={{
                              padding: '4px 3px',
                          ...
                          >
                            {row.peOi ? (row.peOi / 1000).toFixed(0) + 'K' : '—'}
                          </td>
                        </tr>
                      );
                    })}
        ...
        {!chartsExpanded && (
          <div
            ...
            style={{
              width: 5,
              cursor: 'col-resize',
              background: isDragging ? '#5865f2' : 'var(--border)',
              flexShrink: 0,
              transition: isDragging ? 'none' : 'background 0.15s',
            }}
            onMouseEnter={(e) => {
              if (!isDragging) e.currentTarget.style.background = '#5865f2';
            }}
            onMouseLeave={(e) => {
        ...
        >
                    ...
                    ? [
                        {
                          ...
                          color: '#5865f2',
                          dashed: true,
                          label: spot.toLocaleString('en-IN'),
                          labelColor: '#5865f2',
                        },
                      ]
                    : []),
          ...
          {evalResult && (
            ...
            >
              ...
              {!chartsExpanded && (
                <div
                  ref={positionsPaneRef}
                  style={{
                    flexShrink: 0,
                    height: positionsCollapsed ? 28 : positionsHeight,
                    display: 'flex',
                    flexDirection: 'column',
                    borderTop: '1px solid var(--border)',
                    background: 'var(--bg-secondary)',
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text-muted)' }}>
                      ...
                      >
                        {evalResult.grossPnl >= 0 ? '+' : ''}₹{fmtPrice(evalResult.grossPnl)}
                      </span>
                    </span>
                  </div>
                  {!positionsCollapsed && (
                      ...
                      >
                        ...
                        <tbody>
                          {evalResult.legs.map((l, i) => {
                            ...
                            const pnlPct =
                              l.entryPrice > 0
                                ? ((l.exitPrice - l.entryPrice) / l.entryPrice) *
                                  100 *
                                  (side === 'BUY' ? 1 : -1)
                                : 0;
                            const color =
                              l.optionType === 'CALL' || l.optionType === 'CE'
                                ? '#22c55e'
                                : '#ef4444';
                            return (
                              <tr
                                key={i}
                                style={{
                                  borderBottom: '1px solid var(--border)/50',
                                  background: 'var(--bg-primary)',
                                  height: 28,
                                }}
                              >
          ...
          {!evalResult && legs.length === 0 && (
            ...
            >
              <svg viewBox="0 0 300 140" style={{ width: 280, height: 140, opacity: 0.5 }}>
                <defs>
                  <linearGradient id="nbGreen" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#22c55e" stopOpacity={0.3} />
                    <stop offset="100%" stopColor="#22c55e" stopOpacity={0} />
                  </linearGradient>
                </defs>
                <path
                  d="M0,120 L30,100 L60,110 L90,70 L120,80 L150,40 L180,50 L210,30 L240,25 L270,20 L300,18"
                  fill="none"
                  stroke="#22c55e"
                  strokeWidth="2"
                />
                <path
                  d="M0,120 L30,100 L60,110 L90,70 L120,80 L150,40 L180,50 L210,30 L240,25 L270,20 L300,18 L300,140 L0,140 Z"
                  fill="url(#nbGreen)"
                />
                <line
                  x1="0"
              ...
              <div style={{ fontSize: 12, maxWidth: 300, textAlign: 'center', lineHeight: 1.5 }}>
                Pick a date, click on CE/PE prices in the chain to add legs, then simulate to see
