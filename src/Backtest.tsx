...
import {
  computeMetrics as recomputeMetrics,
  buildEquityCurve as recomputeEquity,
  monthlyBreakdown as recomputeMonthly,
  weekdayBreakdown as recomputeWeekday,
  withRecumulated,
  tradeWeekday,
  tradeDte,
...
type ResultTab =
  'trades' | 'monthly' | 'weekday' | 'montecarlo' | 'score' | 'sweep' | 'walkforward';
...
const TEMPLATES: Record<string, () => Leg[]> = {
  'Short Straddle': () => [
    newLeg({
      ...
      stopLoss: { type: 'PREMIUM_PERCENT', value: 30 },
    }),
    newLeg({
      optionType: 'PUT',
      side: 'SELL',
  ...
  'Short Strangle': () => [
    newLeg({
      optionType: 'CALL',
      side: 'SELL',
      strike: { method: 'POINTS_FROM_SPOT', pointsFromSpot: 200 },
      stopLoss: { type: 'PREMIUM_PERCENT', value: 40 },
    }),
    newLeg({
      optionType: 'PUT',
      side: 'SELL',
  ...
  'Iron Condor': () => [
    ...
    newLeg({
      optionType: 'CALL',
      side: 'BUY',
      strike: { method: 'POINTS_FROM_SPOT', pointsFromSpot: 400 },
    }),
    newLeg({
      optionType: 'PUT',
      side: 'SELL',
...
export default function Backtest({ instrument }: Props) {
  ...
  const [config, setConfig] = useState<BacktestConfig>(() =>
    makeConfig('NIFTY', undefined, TEMPLATES['Short Straddle']()),
  );
  ...
  function applyTemplate(name: string) {
    patch({ legs: TEMPLATES[name]() });
  }
  function updateLeg(i: number, leg: Leg) {
    patch({ legs: config.legs.map((l, j) => (j === i ? leg : l)) });
  }
  function removeLeg(i: number) {
    patch({ legs: config.legs.filter((_, j) => j !== i) });
  }
  function addLeg() {
    patch({ legs: [...config.legs, newLeg()] });
  }
  // Scale the lots of EVERY leg by a factor (rounded to whole lots, min 1).
  function multiplyLots(factor: number) {
    patch({
      legs: config.legs.map((l) => ({ ...l, lots: Math.max(1, Math.round(l.lots * factor)) })),
    });
  }
  ...
  const setSizing = (patch: Partial<typeof sz>) =>
    setConfig((c) => ({ ...c, sizing: { ...sz, ...patch } }));
  ...
  const setSquareOffAll = (on: boolean) =>
    setConfig((c) => {
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div style={{ width: panelW }} className="shrink-0 overflow-y-auto p-3 flex flex-col gap-3">
        ...
        <div className="flex gap-1">
          {(
            meta?.underlyings ??
            ([{ underlying: 'NIFTY' }, { underlying: 'SENSEX' }] as UnderlyingMeta[])
          ).map((u) => (
            <button
              key={u.underlying}
              onClick={() => switchUnderlying(u.underlying)}
        ...
        <div className="grid grid-cols-2 gap-2">
          <label>
            ...
            <input
              type="date"
              value={config.from}
              min={undMeta?.firstExpiry}
              max={undMeta?.lastExpiry}
          ...
          <label>
            <span className={lblCls}>Entry time</span>
            <input
              type="time"
          ...
          <label>
            <span className={lblCls}>Exit time</span>
            <input
              type="time"
              value={config.exitTime}
              onChange={(e) => patch({ exitTime: e.target.value })}
              className={inputCls}
            />
          </label>
          <label>
            <span className={lblCls}>Lot size</span>
            <input
              type="number"
              value={config.lotSize}
              onChange={(e) => patch({ lotSize: Number(e.target.value) })}
              className={inputCls}
            />
          </label>
          <label>
            <span className={lblCls}>Slippage %</span>
            <input
              type="number"
              step={0.05}
        ...
        <div className="grid grid-cols-2 gap-2">
          <label>
            <span className={lblCls}>Max profit ₹ (0=off)</span>
            <input
              type="number"
              value={pr.maxProfit ?? 0}
              onChange={(e) =>
                patch({ portfolioRisk: { ...pr, maxProfit: Number(e.target.value) || undefined } })
              }
              className={inputCls}
            />
          </label>
          <label>
            <span className={lblCls}>Max loss ₹ (0=off)</span>
            <input
              type="number"
              value={pr.maxLoss ?? 0}
              onChange={(e) =>
                patch({ portfolioRisk: { ...pr, maxLoss: Number(e.target.value) || undefined } })
              }
              className={inputCls}
            />
          </label>
        ...
        <label className="flex items-start gap-2 cursor-pointer select-none">
          ...
          <span className="text-[11px] leading-tight">
            ...
            <span className="block text-[10px] text-[var(--text-muted)]">
              When one leg hits its stop-loss, square off all remaining legs at the same instant
              (AlgoTest "Complete" square-off).
            </span>
          </span>
        </label>
        ...
        <div>
          ...
          >
            ...
            {activeFilterCount > 0 && (
              <span className="text-[var(--accent)]">({activeFilterCount})</span>
            )}
          </button>
          {showFilters && (
            <div className="mt-1.5 grid grid-cols-2 gap-2">
              {(
                [
                  ...
                  ['ivMin', 'IV min'],
                  ['ivMax', 'IV max'],
                  ['premiumMin', 'Combined prem min'],
                  ['premiumMax', 'Combined prem max'],
                  ['waitTradePct', 'Wait & trade %'],
                ] as [keyof typeof ef, string][]
              ).map(([key, label]) => (
                <label key={key}>
                  <span className={lblCls}>{label} (0=off)</span>
                  <input
                    type="number"
                    value={ef[key] ?? 0}
                    onChange={(e) =>
                      patch({ entryFilters: { ...ef, [key]: Number(e.target.value) || undefined } })
                    }
                    className={inputCls}
                  />
                </label>
        ...
        <div>
          ...
          >
            ...
            {sz.mode !== 'FIXED' && (
              <span className="text-[var(--accent)]">({sz.mode.split('_')[0].toLowerCase()})</span>
            )}
          </button>
          {showSizing && (
            <div className="mt-1.5 flex flex-col gap-2">
              <label>
                <span className={lblCls}>Mode</span>
                <select
                  value={sz.mode}
                  onChange={(e) => setSizing({ mode: e.target.value as SizingMode })}
                  className={inputCls}
                >
              ...
              {sz.mode === 'CAPITAL_PERCENT' && (
                <div className="grid grid-cols-2 gap-2">
                  <label>
                    <span className={lblCls}>Capital ₹</span>
                    <input
                      type="number"
                      value={sz.capital ?? 0}
                      onChange={(e) => setSizing({ capital: Number(e.target.value) || undefined })}
                      className={inputCls}
                    />
                  </label>
                  <label>
                    <span className={lblCls}>Risk % / day</span>
                    <input
                      type="number"
                      value={sz.riskPct ?? 0}
                      onChange={(e) => setSizing({ riskPct: Number(e.target.value) || undefined })}
                      className={inputCls}
                    />
                  </label>
                </div>
              ...
              {sz.mode === 'VOLATILITY_TARGET' && (
                <label>
                  <span className={lblCls}>Baseline IV (lots = base at this IV)</span>
                  <input
                    type="number"
                    value={sz.baselineIv ?? 0}
                    onChange={(e) => setSizing({ baselineIv: Number(e.target.value) || undefined })}
                    className={inputCls}
                  />
                </label>
              )}
              {sz.mode === 'MARTINGALE' && (
                <div className="grid grid-cols-2 gap-2">
                  <label>
                    <span className={lblCls}>Factor / loss</span>
                    <input
                      type="number"
                      step={0.1}
                      value={sz.factor ?? 2}
                      onChange={(e) => setSizing({ factor: Number(e.target.value) || undefined })}
                      className={inputCls}
                    />
                  </label>
                  <label>
                    <span className={lblCls}>Max lots ×</span>
                    <input
                      type="number"
                      value={sz.maxLots ?? 0}
                      onChange={(e) => setSizing({ maxLots: Number(e.target.value) || undefined })}
                      className={inputCls}
                    />
                  </label>
                </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={addLeg}
            className="px-2 py-0.5 rounded text-[11px] bg-[var(--accent)]/20 text-[var(--accent)] hover:bg-[var(--accent)]/30"
          >
            + Add leg
          </button>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          {!config.legs.length && (
            <div className="text-[11px] text-[var(--text-muted)] italic">
              No legs — add one or pick a template.
            </div>
          )}
        </div>
        ...
        <button
          onClick={run}
          disabled={loading || !config.legs.length}
        ...
        {error && (
          <div className="text-[var(--red)] text-[11px] bg-[var(--red)]/10 rounded px-2 py-1.5">
            {error}
          </div>
        )}
      </div>
...
function exportCSV(resp: BacktestResponse) {
  const hdr =
    'Date,Entry Spot,Exit Spot,Margin,ROI%,Gross P&L,Costs,Net P&L,Cum P&L,Exit Reason,Legs\n';
  const rows = resp.trades
    .map((t) => {
      const legs = t.legs
        .map(
          (l) =>
            `${l.side} ${l.optionType} ${l.strike} @${l.entryPrice}→${l.exitPrice} hi=${l.highAfterEntry} lo=${l.lowAfterEntry} ${l.exitReason}`,
        )
        .join(' | ');
    ...
    .join('\n');
  downloadFile(
    `backtest_${resp.config.underlying}_${resp.config.from}_${resp.config.to}.csv`,
    hdr + rows,
    'text/csv',
  );
}
function exportJSON(resp: BacktestResponse) {
  downloadFile(
    `backtest_${resp.config.underlying}_${resp.config.from}_${resp.config.to}.json`,
...
function GradeBadge({ score }: { score: StrategyScore }) {
  return (
    <div className="flex items-center gap-2">
      <span
        className="text-[22px] font-black rounded-md px-2 py-0.5"
        style={{ color: gradeColor[score.grade], background: `${gradeColor[score.grade]}18` }}
      >
        {score.grade}
      </span>
...
function Results({
  ...
  const filtered = useMemo(() => {
    ...
    return resp.trades.filter((t) => {
      if (wkFilter.size) {
        const w = tradeWeekday(t.date);
        if (!w || !wkFilter.has(w)) return false;
      }
      if (dteFilter.size) {
        const d = tradeDte(t);
        if (d == null || !dteFilter.has(d)) return false;
      }
      return true;
    });
  ...
  const m = useMemo(
    () => (isFiltered ? recomputeMetrics(filtered) : resp.metrics),
    [filtered, isFiltered, resp.metrics],
  );
  const equity = useMemo(
    () => (isFiltered ? recomputeEquity(filtered) : resp.equityCurve),
    [filtered, isFiltered, resp.equityCurve],
  );
  const monthly = useMemo(
    () => (isFiltered ? recomputeMonthly(filtered) : resp.monthly),
    [filtered, isFiltered, resp.monthly],
  );
  const weekday = useMemo(
    () => (isFiltered ? recomputeWeekday(filtered) : resp.weekday),
    [filtered, isFiltered, resp.weekday],
  );
  const tradeRows = useMemo(
    () => (isFiltered ? withRecumulated(filtered) : resp.trades),
    [filtered, isFiltered, resp.trades],
  );
  ...
  const onChartDrag = useCallback(
    (e: React.MouseEvent) => {
      ...
      const startH = chartH;
      const onMove = (ev: MouseEvent) =>
        setChartH(Math.min(560, Math.max(110, startH + (ev.clientY - startY))));
      const onUp = () => {
        document.removeEventListener('mousemove', onMove);
        document.removeEventListener('mouseup', onUp);
      };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] text-[var(--text-muted)]">
              {resp.tradingDaysScanned} days scanned
              {isFiltered ? ` · ${filtered.length} of ${resp.trades.length} trades shown` : ''}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <label
              className="flex items-center gap-1"
              title="Maps Overall profit → an estimated AlgoTest figure. Our premiums are cash/spot-priced; AlgoTest prices off futures, so short-premium P&L overshoots. Measured deviation: ≈ −3% (no SL) to −20% (SL + re-entry); hedged structures can differ in sign. Tune to your strategy."
            >
            ...
            <button
              onClick={() => exportCSV(resp)}
              className="px-2 py-0.5 rounded text-[10px] bg-[var(--bg-card)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
            >
              CSV
            </button>
            <button
              onClick={() => exportJSON(resp)}
              className="px-2 py-0.5 rounded text-[10px] bg-[var(--bg-card)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
            >
              JSON
            </button>
        ...
        <div className="grid grid-cols-6 gap-x-3 gap-y-1">
          {cards.map((c) => (
            <div key={c.label} title={c.hint}>
              ...
              <div className={`text-[13px] font-semibold ${c.cls ?? 'text-[var(--text-primary)]'}`}>
                {c.val}
              </div>
              {c.sub && (
                <div
                  className="text-[10px] text-[var(--accent)] tabular-nums leading-tight"
                  title="Estimated AlgoTest result = Overall × (1 + adj%). Calibration is approximate — see the AlgoTest adj field."
                >
                  {c.sub}
                </div>
              )}
      ...
      <div className="shrink-0 flex gap-1 px-3 py-1 border-b border-[var(--border)]">
        {(
          ['trades', 'monthly', 'weekday', 'montecarlo', 'score', 'sweep', 'walkforward'] as const
        ).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
      ...
      {detail && (
        <TradeDetailModal trade={detail} config={config} onClose={() => setDetail(null)} />
      )}
    </div>
  );
}
...
function AdjustmentPanel({
  ...
  config: BacktestConfig;
  patch: (p: Partial<BacktestConfig>) => void;
}) {
  ...
  function addAdj() {
    const adj: Adjustment = {
      ...
      trigger: 'ON_ANY_LEG_SL',
      replacementLegs: [
        newLeg({ optionType: 'CALL', side: 'SELL' }),
        newLeg({ optionType: 'PUT', side: 'SELL' }),
      ],
      maxAdjustments: 1,
      delayBars: 0,
    };
        ...
        <div className="flex flex-col gap-2 pl-1 border-l-2 border-[var(--accent)]/30">
          {visible.map(([adj, i]) => (
            <AdjustmentCard
              ...
              adj={adj}
              index={i}
              onChange={(a) => updateAdj(i, a)}
              onRemove={() => removeAdj(i)}
            />
          ))}
          <button
            onClick={addAdj}
...
function AdjustmentCard({
  ...
  return (
    <div
      className={`rounded-lg border p-2 flex flex-col gap-2 ${adj.enabled ? 'border-orange-500/40 bg-orange-500/5' : 'border-[var(--border)] opacity-50'}`}
    >
      <div className="flex items-center gap-2">
        <span className="text-[10px] text-orange-400 font-mono">ADJ{index + 1}</span>
        <select
          value={adj.trigger}
          onChange={(e) => set({ trigger: e.target.value as AdjustmentTrigger })}
        ...
        <button
          onClick={() => set({ enabled: !adj.enabled })}
        ...
        <button
          onClick={onRemove}
          className="px-1.5 py-0.5 rounded text-[12px] text-[var(--text-muted)] hover:text-[var(--red)]"
        >
          ✕
        </button>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={addRLeg}
          className="px-2 py-0.5 rounded text-[10px] bg-[var(--accent)]/20 text-[var(--accent)] hover:bg-[var(--accent)]/30"
        >
          + Add
        </button>
      </div>
      <div className="flex flex-col gap-1.5">
        {rLegs.map((leg, i) => (
          <LegCard
            ...
            leg={leg}
            index={i}
            onChange={(l) => updateRLeg(i, l)}
            onRemove={() => removeRLeg(i)}
          />
        ))}
      </div>
    </div>
        ...
        <thead className="sticky top-0 bg-[var(--bg-secondary)]">
          <tr>
            {headers.map((h) => (
              <th
                key={h}
                className="text-left px-2 py-1.5 text-[var(--text-muted)] font-medium border-b border-[var(--border)] whitespace-nowrap"
              >
                {h}
              </th>
            ))}
        ...
        <tbody>
          {trades.map((t) => (
            <tr
              key={t.date}
              onClick={() => onSelect(t)}
              title="Click for trade detail & intraday P&L"
            ...
            >
              ...
              <td className="px-2 py-1 text-[var(--text-muted)] whitespace-nowrap">
                {Math.round(t.entrySpot)}→{Math.round(t.exitSpot)}
              </td>
              <td className="px-2 py-1 text-[var(--text-muted)] whitespace-nowrap">
                {t.legs.map((l, i) => (
                  <span key={i}>
                    {i > 0 && ' '}
                    <span
                      className={l.side === 'SELL' ? 'text-[var(--red)]' : 'text-[var(--green)]'}
                    >
                      {l.side === 'SELL' ? '-' : '+'}
                      {l.optionType === 'CALL' ? 'CE' : 'PE'} {l.strike}
                    </span>
                    <span className="text-[var(--text-muted)] opacity-60">
                      {' '}
                      @{l.entryPrice.toFixed(1)}→{l.exitPrice.toFixed(1)}
                    </span>
                  </span>
                ))}
              </td>
              ...
              <td
                className={`px-2 py-1 whitespace-nowrap ${t.roiPct >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
              >
                {t.roiPct?.toFixed(2)}%
              </td>
              <td
                className={`px-2 py-1 ${t.grossPnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
              >
                {inr(t.grossPnl)}
              </td>
              <td className="px-2 py-1 text-[var(--text-muted)]">{inr(t.costs)}</td>
              <td
                className={`px-2 py-1 font-semibold ${t.pnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
              >
                {inr(t.pnl)}
              </td>
              <td
                className={`px-2 py-1 ${t.cumPnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
              >
                {inr(t.cumPnl)}
              </td>
              <td className="px-2 py-1" style={{ color: reasonColor(t.exitReason) }}>
                {t.exitReason}
              </td>
            </tr>
          ))}
    ...
    <table className="w-full text-[11px] border-collapse">
      <thead className="sticky top-0 bg-[var(--bg-secondary)]">
        <tr>
          {['Month', 'P&L', 'Trades', 'Win rate'].map((h) => (
            <th
              key={h}
              className="text-left px-3 py-1.5 text-[var(--text-muted)] font-medium border-b border-[var(--border)]"
            >
              {h}
            </th>
          ))}
      ...
      <tbody>
        {monthly.map((b) => (
          <tr
            key={b.month}
            className="border-b border-[var(--border)]/40 hover:bg-[var(--bg-hover)]"
          >
            <td className="px-3 py-1 text-[var(--text-secondary)]">{b.month}</td>
            <td
              className={`px-3 py-1 font-semibold ${b.pnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
            >
              {inr(b.pnl)}
            </td>
...
function WeekdayTable({ weekday }: { weekday: WeekdayBucket[] }) {
  return (
    <table className="w-full text-[11px] border-collapse">
      <thead className="sticky top-0 bg-[var(--bg-secondary)]">
        <tr>
          {['Weekday', 'P&L', 'Trades', 'Win rate'].map((h) => (
            <th
              key={h}
              className="text-left px-3 py-1.5 text-[var(--text-muted)] font-medium border-b border-[var(--border)]"
            >
...
function TradeDetailModal({
  ...
  useEffect(() => {
    ...
    fetch('/api/backtest/day', {
      ...
      .then((d) => {
        if (!alive) return;
        if (!d.ok || !d.series) setError(d.error || 'No intraday data.');
        else setSeries(d.series);
      })
      .catch((e) => {
  ...
  const chartData = useMemo(
    () => (series ?? []).map((p) => ({ hhmm: p.hhmm, total: p.total, spot: p.spot })),
    [series],
  );
  ...
  const summary: { label: string; val: string; cls?: string }[] = [
    ...
    {
      label: 'Gross P&L',
      val: inr(trade.grossPnl),
      cls: trade.grossPnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]',
    },
    {
      label: 'Net P&L',
      val: inr(trade.pnl),
      cls: trade.pnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]',
    },
    {
      label: 'ROI on margin',
      val: `${trade.roiPct.toFixed(2)}%`,
      cls: trade.roiPct >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]',
    },
    {
      label: 'ROI on peak margin',
      val: `${peakRoi.toFixed(2)}%`,
      cls: peakRoi >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]',
    },
  ];
  ...
  return (
    ...
    >
      <div
        className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg shadow-xl w-full max-w-[820px] max-h-[88vh] overflow-y-auto"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--border)] sticky top-0 bg-[var(--bg-secondary)] z-10">
          <div className="flex items-center gap-3">
            ...
            <span
              ...
              style={{
                color: reasonColor(trade.exitReason),
                background: `${reasonColor(trade.exitReason)}1a`,
              }}
            >
              {trade.exitReason}
            </span>
        ...
        <div className="p-4 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-x-4 gap-y-2">
            {summary.map((s) => (
              <div key={s.label}>
                ...
                <div
                  className={`text-[13px] font-semibold tabular-nums ${s.cls ?? 'text-[var(--text-primary)]'}`}
                >
                  {s.val}
                </div>
              </div>
          ...
          {view === 'chart' && (
            <>
              {loading && (
                <div className="flex items-center justify-center h-[200px] text-[11px] text-[var(--text-muted)]">
                  Loading chart…
                </div>
              )}
              {error && (
                <div className="flex items-center justify-center h-[200px] text-[11px] text-[var(--red)]">
                  {error}
                </div>
              )}
              {!loading && !error && series && series.length > 0 && (
                <TradeChartView trade={trade} series={series} underlying={config.underlying} />
              )}
            </>
          )}
                ...
                <div className="h-[220px] border border-[var(--border)] rounded bg-[var(--bg-primary)]/40">
                  {loading && (
                    <div className="flex items-center justify-center h-full text-[11px] text-[var(--text-muted)]">
                      Loading intraday P&amp;L…
                    </div>
                  )}
                  {error && (
                    <div className="flex items-center justify-center h-full text-[11px] text-[var(--red)]">
                      {error}
                    </div>
                  )}
                      ...
                      {trade.legs.map((l, i) => (
                        <tr key={i} className="border-b border-[var(--border)]/40">
                          <td className="px-2 py-1 whitespace-nowrap">
                            <span
                              className={
                                l.side === 'SELL' ? 'text-[var(--red)]' : 'text-[var(--green)]'
                              }
                            >
                              {l.side === 'SELL' ? '-' : '+'}
                              {l.optionType === 'CALL' ? 'CE' : 'PE'} {l.strike}
                          ...
                          <td className="px-2 py-1 text-[var(--text-muted)] whitespace-nowrap">
                            {(l.seq ?? 0) === 0 ? 'Entry' : `Re-entry ${l.seq}`}
                          </td>
                          <td className="px-2 py-1 text-[var(--text-secondary)] whitespace-nowrap">
                            {l.entryTime} @{l.entryPrice.toFixed(1)}
                          </td>
                          <td className="px-2 py-1 text-[var(--text-secondary)] whitespace-nowrap">
                            {l.exitTime} @{l.exitPrice.toFixed(1)}
                          </td>
                          <td className="px-2 py-1 text-[var(--text-muted)] tabular-nums">
                            {Math.round(l.entrySpot)}
                          </td>
                          <td className="px-2 py-1 whitespace-nowrap text-[10px]">
                            <span className="text-[var(--green)]">
                              ↑{l.highAfterEntry?.toFixed(1)}
                            </span>
                            <span className="text-[var(--red)]">
                              {' '}
                              ↓{l.lowAfterEntry?.toFixed(1)}
                            </span>
                          </td>
                          <td
                            className={`px-2 py-1 font-semibold tabular-nums ${l.pnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                          >
                            {inr(l.pnl)}
                          </td>
                          <td className="px-2 py-1" style={{ color: reasonColor(l.exitReason) }}>
                            {l.exitReason}
                          </td>
                        </tr>
                      ))}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-x-4 gap-y-1.5 text-[11px]">
                    ...
                    <div>
                      ...
                      <div className="text-[var(--red)] font-semibold tabular-nums">
                        {inr(ch.total)}
                      </div>
                    </div>
                  </div>
  ...
  if (!mc || !mc.simulations) {
    return (
      <div className="p-4 text-[var(--text-muted)] text-[12px]">
        Not enough trades for Monte Carlo (need 5+).
      </div>
    );
  }
  ...
  return (
    <div className="p-3 flex flex-col gap-3">
      ...
      <table className="text-[11px] border-collapse">
        <thead>
          <tr>
            {['Percentile', 'Final equity', 'Max drawdown'].map((h) => (
              <th
                key={h}
                className="text-left px-3 py-1 text-[var(--text-muted)] font-medium border-b border-[var(--border)]"
              >
                {h}
              </th>
            ))}
        ...
        <tbody>
          {mc.percentiles.map((p) => (
            <tr key={p.pct} className="border-b border-[var(--border)]/40">
              ...
              <td
                className={`px-3 py-1 font-semibold ${p.finalEquity >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
              >
                {inr(p.finalEquity)}
              </td>
...
function ScoreTab({ resp }: { resp: BacktestResponse }) {
  const s = resp.score;
  if (!s)
    return <div className="p-4 text-[var(--text-muted)] text-[12px]">No score available.</div>;
  return (
    <div className="p-3 flex flex-col gap-3">
      <div className="flex items-center gap-3">
        <GradeBadge score={s} />
        <span className="text-[12px] text-[var(--text-secondary)]">
          Weighted composite of 7 factors
        </span>
      </div>
      <div className="flex flex-col gap-1.5">
        {s.breakdown.map((f) => (
            ...
            <div className="flex-1 h-[10px] bg-[var(--bg-card)] rounded overflow-hidden">
              <div
                ...
                style={{
                  ...
                  background: f.score >= 60 ? '#22c55e' : f.score >= 30 ? '#eab308' : '#ef4444',
                }}
              />
            </div>
            <span className="text-[10px] text-[var(--text-muted)] w-[50px] text-right">
              {f.score.toFixed(0)}/100 ({(f.weight * 100).toFixed(0)}%)
            </span>
          </div>
        ))}
      </div>
...
const SWEEP_METRICS: (keyof Metrics)[] = [
  'totalPnl',
  'sharpe',
  'profitFactor',
  'maxDrawdown',
...
function SweepTab({ config }: { config: BacktestConfig }) {
  ...
  const runSweep = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok || !data.ok) {
        setError((data as any).error || 'Sweep failed');
        return;
      }
      setResult(data);
    } catch (e) {
    ...
    <div className="p-3 flex flex-col gap-3">
      <div className="flex items-end gap-2 flex-wrap">
        ...
        <label className="flex flex-col gap-0.5">
          ...
          <select
            value={metric}
            onChange={(e) => setMetric(e.target.value as keyof Metrics)}
        ...
        <>
          <div className="text-[11px] text-[var(--text-secondary)]">
            ...
            <span className="font-semibold text-[var(--accent)]">
              {SWEEP_PRESETS[preset].label} = {result.bestV1}
              {is2D && result.bestV2 !== undefined
                ? `, ${SWEEP_PRESETS[preset2].label} = ${result.bestV2}`
                : ''}
            </span>
            {' → '}
            {metric} ={' '}
            <span className="font-semibold text-[var(--green)]">{result.bestMetric}</span>
          </div>

          {is2D ? (
            <SweepHeatmap
              result={result}
              px={SWEEP_PRESETS[preset].label}
              py={SWEEP_PRESETS[preset2].label}
            />
          ) : (
            <>
              <div className="h-[200px]">
                <SvgChart
                  ...
                  yFormatter={(v) =>
                    typeof v === 'number' && Math.abs(v) >= 1000 ? inr(v) : String(v)
                  }
                  showLegend={false}
                />
              </div>
              <table className="text-[11px] border-collapse">
                ...
                <tbody>
                  {result.cells.map((c, i) => (
                    <tr
                      key={i}
                      className={`border-b border-[var(--border)]/40 ${c.v1 === result.bestV1 ? 'bg-[var(--accent)]/10' : ''}`}
                    >
                      <td className="px-3 py-1 text-[var(--text-secondary)]">{c.v1}</td>
                      <td
                        className={`px-3 py-1 font-semibold ${c.metric >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                      >
                        {typeof c.metric === 'number' && Math.abs(c.metric) >= 100
                          ? inr(c.metric)
                          : c.metric}
                      </td>
...
function SweepHeatmap({ result, px, py }: { result: SweepResponse; px: string; py: string }) {
  ...
  const min = Math.min(...vals),
    max = Math.max(...vals);
  const fmt = (v: number) =>
    Math.abs(v) >= 1000 ? `${(v / 1000).toFixed(0)}k` : String(Math.round(v * 100) / 100);

  return (
    <div className="overflow-auto">
      <div className="text-[10px] text-[var(--text-muted)] mb-1">
        Rows: {py} · Cols: {px} (best highlighted)
      </div>
      <table className="border-collapse text-[10px]">
        <thead>
          <tr>
            <th className="px-1.5 py-1 text-[var(--text-muted)] sticky left-0 bg-[var(--bg-secondary)]">
              {py}\{px}
            </th>
            {xs.map((x) => (
              <th key={x} className="px-1.5 py-1 text-[var(--text-muted)] font-medium">
                {x}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {ys.map((y) => (
            <tr key={y}>
              <td className="px-1.5 py-1 text-[var(--text-secondary)] font-medium sticky left-0 bg-[var(--bg-secondary)]">
                {y}
              </td>
              {xs.map((x) => {
                ...
                return (
                  <td
                    key={x}
                    className={`px-1.5 py-1 text-center ${best ? 'ring-2 ring-white' : ''}`}
                    style={{
                      background: v !== undefined ? heatColor(v, min, max) : 'transparent',
                      color: '#0a0a0a',
                    }}
                  >
                    {v !== undefined ? fmt(v) : '–'}
...
function WalkForwardTab({ config }: { config: BacktestConfig }) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok || !data.ok) {
        setError((data as any).error || 'Walk-forward failed');
        return;
      }
      setResult(data);
    } catch (e) {
      ...
      <div className="flex items-end gap-2 flex-wrap">
        ...
        <label className="flex flex-col gap-0.5 w-[70px]">
          ...
          <input
            ...
            max={80}
            value={oosPct}
            onChange={(e) => setOosPct(Number(e.target.value))}
            className={inputCls}
          />
        </label>
        <button
      ...
      {result && (
        <>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <div className={lblCls}>OOS total P&L</div>
              <div
                className={`text-[14px] font-semibold ${result.oosTotalPnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
              >
                {inr(result.oosTotalPnl)}
              </div>
            </div>
            <div>
              <div className={lblCls}>OOS window win %</div>
              <div className="text-[14px] font-semibold text-[var(--text-primary)]">
                {result.oosWinRate}%
              </div>
            </div>
            <div>
              <div className={lblCls}>Efficiency (OOS/IS)</div>
              <div
                className={`text-[14px] font-semibold ${result.efficiency >= 0.5 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
              >
                {result.efficiency}
              </div>
            </div>
            ...
            <thead>
              <tr>
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-2 py-1 text-[var(--text-muted)] font-medium border-b border-[var(--border)]"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {result.windows.map((w) => (
                <tr key={w.index} className="border-b border-[var(--border)]/40">
                  ...
                  <td className="px-2 py-1 text-[var(--text-muted)] whitespace-nowrap">
                    {w.isFrom.slice(2)}→{w.isTo.slice(2)}
                  </td>
                  ...
                  <td className="px-2 py-1 text-[var(--text-muted)] whitespace-nowrap">
                    {w.oosFrom.slice(2)}→{w.oosTo.slice(2)}
                  </td>
                  <td
                    className={`px-2 py-1 font-semibold ${w.oosPnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                  >
                    {inr(w.oosPnl)}
                  </td>
