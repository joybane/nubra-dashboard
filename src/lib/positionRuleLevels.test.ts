...
describe('liveLevels — PREMIUM_PRICE (absolute level)', () => {
  it('returns the level verbatim regardless of side', () => {
    const buy  = liveLevels('BUY',  82.7, { type: 'PREMIUM_PRICE', value: 81.5 }, { type: 'PREMIUM_PRICE', value: 84 });
    expect(buy).toEqual({ slPrice: 81.5, tgtPrice: 84 });

    const sell = liveLevels('SELL', 82.7, { type: 'PREMIUM_PRICE', value: 90 }, { type: 'PREMIUM_PRICE', value: 60 });
    expect(sell).toEqual({ slPrice: 90, tgtPrice: 60 });
  });

  it('coerces numeric strings, which JSON round-trips can produce', () => {
    const r = liveLevels('BUY', 82.7, NONE, { type: 'PREMIUM_PRICE', value: '84' as unknown as number });
...
describe('liveLevels — parity with the backtest engine (premiumLevels)', () => {
  ...
  it('PREMIUM_ABSOLUTE is an offset from entry — adverse for SL, favourable for target', () => {
    expect(liveLevels('BUY', 100, { type: 'PREMIUM_ABSOLUTE', value: 20 }, { type: 'PREMIUM_ABSOLUTE', value: 30 }))
      .toEqual({ slPrice: 80, tgtPrice: 130 });
    expect(liveLevels('SELL', 100, { type: 'PREMIUM_ABSOLUTE', value: 20 }, { type: 'PREMIUM_ABSOLUTE', value: 30 }))
      .toEqual({ slPrice: 120, tgtPrice: 70 });
  });

  it('PREMIUM_PERCENT is a percentage of entry premium', () => {
    expect(liveLevels('BUY', 100, { type: 'PREMIUM_PERCENT', value: 25 }, { type: 'PREMIUM_PERCENT', value: 50 }))
      .toEqual({ slPrice: 75, tgtPrice: 150 });
    expect(liveLevels('SELL', 100, { type: 'PREMIUM_PERCENT', value: 25 }, { type: 'PREMIUM_PERCENT', value: 50 }))
  ...
  it('treats NONE and a missing value as "no level"', () => {
    ...
    expect(liveLevels('BUY', 100, { type: 'PREMIUM_PRICE' }, { type: 'PREMIUM_ABSOLUTE' }))
      .toEqual({ slPrice: null, tgtPrice: null });
  });
});
...
describe('the reported bug — NIFTY 24000 CE, BUY @ 82.70, LTP 84.75', () => {
  ...
  it('the same 84 as PREMIUM_ABSOLUTE resolves to 166.70 and does not fire', () => {
    const { slPrice, tgtPrice } = liveLevels(
      'BUY', ENTRY, { type: 'PREMIUM_ABSOLUTE', value: 81.5 }, { type: 'PREMIUM_ABSOLUTE', value: 84 },
    );
