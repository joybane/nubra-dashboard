...
export default function SavedStrategiesTab({ onOpen }: Props) {
  ...
  const del = useCallback(
    async (id: string) => {
      try {
        await fetch(`/paper/strategy/snapshot/${id}`, { method: 'DELETE' });
      } catch (e) {
        console.warn('[SavedStrategies] delete failed:', e);
      }
      setConfirmId(null);
      load();
    },
  ...
  if (snaps.length === 0) {
    return (
      <div className="flex h-full items-center justify-center px-6 text-center text-[12px] text-[var(--text-muted)]">
        No saved strategies yet. Open a strategy chart and click{' '}
        <span className="mx-1 font-semibold text-[var(--text-primary)]">Save</span>, or they'll be
        captured automatically after market close.
      </div>
    );
  }
            ...
            return (
              ...
              >
                ...
                <td className="px-3 py-1.5 font-medium text-[var(--text-primary)]">
                  {s.strategy_name || s.basket_group_id}
                </td>
                ...
                <td
                  className={`px-3 py-1.5 text-right font-semibold ${pnl >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                >
                  {pnl >= 0 ? '+' : '-'}₹{fmtPrice(Math.abs(pnl))}
                </td>
                <td className="px-3 py-1.5 text-[10px] uppercase text-[var(--text-muted)]">
                  {s.source}
                </td>
                <td className="px-3 py-1.5">
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    {confirmId === s.snapshot_id ? (
                      ...
                      <button
                        onClick={() => setConfirmId(s.snapshot_id)}
