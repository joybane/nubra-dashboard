...
export default function OrderTicket() {
  ...
  useEffect(() => {
    if (ticketOpen) fetchMargin();
  }, [ticketOpen, fetchMargin]);
  ...
  async function placeOrder() {
    if (!instrument) {
      setResult({ ok: false, msg: 'Select an instrument first.' });
      return;
    }
    ...
    if (!nubraName && instrument.ref_id) {
      try {
        ...
        if (data.instrument) {
          ...
          if (m.derivative_type && !instrument.derivative_type)
            instrument.derivative_type = m.derivative_type;
    ...
    if (!nubraName) {
      setResult({ ok: false, msg: 'Instrument has no canonical name. Re-search and select.' });
      return;
    }
    if (lots < 1) {
    ...
    const apiTrigger =
      orderType === 'SL' && triggerPx ? Math.round(Number(triggerPx) * 100) : undefined;
    if (orderType === 'SL' && !apiTrigger) {
      setResult({ ok: false, msg: 'Enter a trigger price.' });
      return;
    }
    if (orderType === 'LIMIT' && !apiPrice) {
      setResult({ ok: false, msg: 'Enter a limit price.' });
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div
        className="relative overflow-hidden flex flex-col max-h-[90vh]"
        style={{
          width: 420,
          background: '#1c1f2e',
          borderRadius: 12,
      ...
      >
        ...
        <div
          ...
          style={{ borderBottom: '1px solid #2a2d3e', cursor: 'grab', userSelect: 'none' }}
        >
          <button
            onClick={() => setSide('BUY')}
            style={{
              ...
              padding: '3px 8px',
              borderRadius: 4,
              background: isBuy ? '#2563eb' : '#252836',
              color: isBuy ? '#fff' : '#60a5fa',
              cursor: 'pointer',
              border: 'none',
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              padding: '3px 8px',
              borderRadius: 4,
              background: !isBuy ? '#dc2626' : '#252836',
              color: !isBuy ? '#fff' : '#f87171',
              cursor: 'pointer',
              border: 'none',
            }}
          >
          ...
          >
            ...
            {chgRs != null && (
              <span style={{ fontSize: 12, color: chgUp ? '#4ade80' : '#f87171' }}>
                • {chgUp ? '+' : ''}
                {chgRs.toFixed(2)} {chgPct != null && `(${chgPct.toFixed(2)}%)`}
              </span>
            )}
        ...
        <div className="overflow-y-auto">
          ...
          {instrument && (
            <>
              ...
              <div className="px-4 pt-4 pb-3">
                <div className="flex gap-3 mb-3">
                  <div className="flex-1">
                    ...
                    <div
                      className="flex items-center h-9 rounded overflow-hidden"
              ...
              {/* ── SL / Target toggles ── */}
              <div
                className="px-4 pb-3 flex gap-6 pt-3"
                style={{ borderTop: '1px dashed #2a2d3e' }}
              >
                    ...
                    <div className="flex gap-5">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="radio"
                          name="validity"
                          checked={validity === 'AMO'}
                          onChange={() => setValidity('AMO')}
                          className="accent-blue-500"
                        />
