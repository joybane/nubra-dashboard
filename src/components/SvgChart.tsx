...
export default function SvgChart({
  ...
  const fmtX =
    xFormatter || ((v: any) => (typeof v === 'number' ? v.toLocaleString('en-IN') : String(v)));
              ...
              <g key={`ref${i}`}>
                ...
                {r.label && (
                  <text
                    ...
                    y={y - 4}
                    textAnchor="end"
                    fill={r.labelColor || r.color || axisColor}
                    fontSize={10}
                  >
                    {r.label}
                  </text>
            ...
            <g key={`ref${i}`}>
              ...
              {r.label && (
                <text
                  x={x + 4}
                  y={m.top + 12}
                  fill={r.labelColor || r.color || '#5865f2'}
                  fontSize={10}
                >
                  {r.label}
                </text>
          ...
          <g transform={`translate(${m.left + 8}, ${size.h - legendH + 4})`}>
            {series.map((s, i) => (
              <g key={s.dataKey} transform={`translate(${i * 120}, 0)`}>
                ...
                />
                <text x={18} y={10} fill={axisColor} fontSize={10}>
                  {legendLabels?.[s.dataKey] || s.dataKey}
                </text>
              </g>
            ))}
          </g>
      ...
      {hover && (
        ...
        >
          {tooltipFormatter ? (
            ...
            <>
              <div style={{ fontWeight: 600, marginBottom: 2, color: '#8b8fa3' }}>
                {fmtX(data[hover.idx]?.[xKey])}
              </div>
              {series.map((s) => (
                <div key={s.dataKey} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontWeight: 600 }}>
                    {data[hover.idx][s.dataKey]?.toFixed(2) ?? '—'}
                  </span>
                </div>
              ))}
            </>
