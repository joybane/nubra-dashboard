...
// Each tooltip comes in two pieces: a `*Body` that renders the card from plain props, and the
// original `forwardRef` wrapper that owns visibility/position and drives the body from its own
// state. The split exists because a pane can now show more than one card at a time — the hover
// tooltip keeps using the imperative wrapper, while pinned crosshairs render the body directly.
// The wrappers' ref API and markup are unchanged.

export interface PriceTooltipData {
  timeStr: string;
  ohlc: { o: number; h: number; l: number; c: number } | null;
  legPrices: Array<{ name: string; color: string; value: number }>;
  underlying: string;
}
    ...
    <div className="bg-[#1a1e24]/75 border border-[#ffffff08] rounded-lg px-3 py-2 shadow-xl backdrop-blur-md min-w-[190px]">
      ...
      {ohlc && (
        <div className="text-[11px] mb-1">
          ...
          <span className={ohlc.c >= ohlc.o ? 'text-[var(--green)]' : 'text-[var(--red)]'}>
            {fmtPrice(ohlc.o)}
          </span>{' '}
          <span className="text-[var(--text-muted)]">H</span>{' '}
          <span className="text-[var(--green)]">{fmtPrice(ohlc.h)}</span>{' '}
          <span className="text-[var(--text-muted)]">L</span>{' '}
          <span className="text-[var(--red)]">{fmtPrice(ohlc.l)}</span>{' '}
...
export const PriceTooltip = memo(
  forwardRef<PriceTooltipRef, {}>((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      setData: (timeStr, ohlc, legPrices, underlying) =>
        setData({ timeStr, ohlc, legPrices, underlying }),
    ...
    return (
      <div
        ...
        style={{ display: visible && (ohlc || legPrices.length > 0) ? 'block' : 'none' }}
      >
        <PriceTooltipBody
          timeStr={timeStr}
          ohlc={ohlc}
...
export interface PnlTooltipValues {
  legs: Array<{ name: string; color: string; value: number }>;
  total: number;
}

export function PnlTooltipBody({
  timeStr,
  values,
  strategyMargin,
}: {
  timeStr: string;
  values: PnlTooltipValues | null;
      ...
      {values &&
        values.legs.map((l: any) => (
          <div key={l.name} className="flex items-center justify-between gap-4 text-[11px] py-0.5">
            ...
            <span
              className={`font-medium tabular-nums ${l.value >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
            >
            ...
            <>
              <div className="flex items-center justify-between gap-4 text-[11px] pt-0.5">
                ...
                <span className="text-[var(--text-secondary)] tabular-nums">
                  ₹{fmtPrice(strategyMargin)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-4 text-[11px] pt-0.5">
                <span className="text-[var(--text-muted)]">ROI</span>
                <span
                  className={`font-medium tabular-nums ${values.total >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                >
                  {((values.total / strategyMargin) * 100).toFixed(2)}%
                </span>
              </div>
            </>
          )}
...
export const PnlTooltip = memo(
  forwardRef<PnlTooltipRef, { strategyMargin: number }>((props, ref) => {
    const [visible, setVisible] = useState(false);
    const containerRef = useRef<HTMLDivElement>(null);
    ...
    return (
      <div
        ref={containerRef}
        className="absolute z-50 pointer-events-none top-0 left-0"
        style={{ display: visible && values ? 'block' : 'none' }}
      >
        <PnlTooltipBody timeStr={timeStr} values={values} strategyMargin={props.strategyMargin} />
      </div>
    );
  }),
);

export function GreeksTooltipBody({
  timeStr,
  values,
  selectedGreeks,
  greeksLegFilter,
  ...
  selectedGreeks: Set<string>;
  greeksLegFilter: Set<string>;
      ...
      {values &&
        (greeksLegFilter.size > 1 ? (
                  ...
                  <tr key={src} className="border-b border-[#ffffff05] last:border-0">
                    ...
                    {['delta', 'gamma', 'theta', 'vega'].map((k) => {
                      ...
                      return (
                        <td
                          key={k}
                          className={`text-right font-semibold py-1 px-1 font-mono ${val >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                        >
                          {val >= 0 ? '+' : ''}
                          {formatted}
                        </td>
          ...
          (() => {
            ...
            return (['delta', 'gamma', 'theta', 'vega'] as const)
              .filter((k) => selectedGreeks.has(k) && d[k] != null)
              .map((k) => {
                const val = d[k] ?? 0;
                const formatted = k === 'gamma' ? val.toFixed(4) : val.toFixed(2);
                return (
                  ...
                  >
                    ...
                    <span
                      className={`font-semibold tabular-nums ${val >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                    >
                      {val >= 0 ? '+' : ''}
                      {formatted}
                    </span>
...
export const GreeksTooltip = memo(
  forwardRef<
    GreeksTooltipRef,
    { selectedGreeks: Set<string>; greeksLegFilter: Set<string>; colors: Record<string, string> }
  >((props, ref) => {
    const [visible, setVisible] = useState(false);
    const containerRef = useRef<HTMLDivElement>(null);
    const [data, setData] = useState({ timeStr: '', values: null as any });
