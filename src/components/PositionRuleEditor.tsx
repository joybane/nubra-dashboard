...
export default function PositionRuleEditor(props: Props) {
  ...
  function levelHint(kind: 'SL' | 'TARGET', level: number | null) {
    ...
    return (
      <div
        className={`text-[10px] mt-0.5 ${warn ? 'text-[var(--yellow)]' : 'text-[var(--text-muted)]'}`}
      >
        → exits when LTP {cmp} {rs(level)}
        {warn ? ` — ${warn}` : ''}
      </div>
    );
  }
    ...
    try {
      if (props.mode === 'LEG') {
        await fetch('/paper/positions/rules/leg', {
          ...
          body: JSON.stringify({
            ref_id: props.refId,
            basket_group_id: props.basketGroupId,
            stopLoss,
            target,
            trail,
      ...
      console.warn('[PositionRuleEditor] save failed:', e);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function clearRule() {
    ...
    try {
      if (props.mode === 'LEG') {
        await fetch(
          `/paper/positions/rules/leg?ref_id=${props.refId}&basket_group_id=${encodeURIComponent(props.basketGroupId)}`,
          { method: 'DELETE' },
        );
      } else {
        await fetch(
          `/paper/positions/rules/group?basket_group_id=${encodeURIComponent(props.basketGroupId)}`,
          { method: 'DELETE' },
        );
      }
      props.onSaved();
      ...
      console.warn('[PositionRuleEditor] clear failed:', e);
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    ...
    >
      <div
        className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg shadow-xl w-96 p-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-3">
          <span className="text-[13px] font-semibold text-[var(--text-primary)]">
            {props.mode === 'LEG'
              ? `SL/Target — ${props.displayName}`
              : `Group SL/Target — ${props.strategyName}`}
          </span>
          <button
            onClick={props.onClose}
        ...
        <div className="flex flex-col gap-2">
          {props.mode === 'LEG' ? (
            <div className="flex flex-col gap-2">
              <div className="text-[10px] text-[var(--text-muted)] -mt-1">
                Entry {rs(entryRs)} ·{' '}
                <span className={side === 'BUY' ? 'text-[var(--green)]' : 'text-[var(--red)]'}>
                  {side}
                </span>
                ...
                <div className="flex gap-1.5">
                  <label className="flex-1 min-w-0">
                    ...
                    <select
                      value={stopLoss.type}
                      onChange={(e) =>
                        setStopLoss({ ...stopLoss, type: e.target.value as LiveSLTargetType })
                      }
                      className={inputCls}
                    >
                      {SL_TYPES.map((s) => (
                  ...
                  {stopLoss.type !== 'NONE' && (
                    <label className="w-20 shrink-0">
                      ...
                      <input
                        type="number"
                        step="any"
                        value={stopLoss.value ?? 0}
                        onChange={(e) =>
                          setStopLoss({ ...stopLoss, value: Number(e.target.value) })
                        }
                        className={inputCls}
                      />
                    </label>
                ...
                <div className="flex gap-1.5">
                  <label className="flex-1 min-w-0">
                    ...
                    <select
                      value={target.type}
                      onChange={(e) =>
                        setTarget({ ...target, type: e.target.value as LiveSLTargetType })
                      }
                      className={inputCls}
                    >
                      {SL_TYPES.map((s) => (
            ...
            <div className="flex flex-col gap-2">
              ...
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={exitAllOnLegHit}
                  onChange={(e) => setExitAllOnLegHit(e.target.checked)}
                />
                <span className="text-[11px] text-[var(--text-primary)]">
                  Exit all legs if any single leg hits its own SL/Target
                </span>
              </label>
            </div>
          )}
          ...
          <label>
            <span className={lblCls}>
              {props.mode === 'LEG' ? 'Trailing stop' : 'Trailing profit lock (combined ₹)'}
            </span>
            <select
              value={trail.type}
              onChange={(e) => setTrail({ ...trail, type: e.target.value as LiveTrailType })}
              className={inputCls}
            >
              {TRAIL_TYPES.map((t) => (
            ...
            <div className="grid grid-cols-3 gap-1">
              <label>
                ...
                <input
                  ...
                  onChange={(e) => setTrail({ ...trail, trigger: Number(e.target.value) })}
                  className={inputCls}
                />
              </label>
              {(trail.type === 'LOCK' || trail.type === 'LOCK_AND_TRAIL') && (
                <label>
                  ...
                  <input
                    type="number"
                    value={trail.lock ?? 0}
                    onChange={(e) => setTrail({ ...trail, lock: Number(e.target.value) })}
                    className={inputCls}
                  />
                </label>
              )}
                ...
                <>
                  <label>
                    ...
                    <input
                      type="number"
                      value={trail.step ?? 0}
                      onChange={(e) => setTrail({ ...trail, step: Number(e.target.value) })}
                      className={inputCls}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      type="number"
                      value={trail.trail ?? 0}
                      onChange={(e) => setTrail({ ...trail, trail: Number(e.target.value) })}
                      className={inputCls}
                    />
                  </label>
                </>
        ...
        {props.mode === 'GROUP' && trail.type !== 'NONE' && (
          <p className="text-[10px] text-[var(--text-muted)] mt-1">
            e.g. trigger 1000, lock 500 → once combined profit reaches ₹1000, exit if it drops back
            to ₹500.
          </p>
        )}
