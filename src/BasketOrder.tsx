...
function miniPayoff(
  legs: Array<{ optionType: 'CE' | 'PE'; side: 'BUY' | 'SELL'; strikeDist: number; lots: number }>,
  ...
  return Array.from({ length: 41 }, (_, i) => ({
    x: i,
    y: payoffAtExpiry(min + i * step, mapped),
  }));
}
...
export default function BasketOrder({ instrument }: Props) {
  ...
  const instSym = instrument
    ? instrument.asset ||
      (instrument.stock_name || '').replace(/\s+\d+$/, '').replace(/\s+/g, '') ||
      getSymbol(instrument).replace(/\d.*/, '') ||
  ...
  function addLeg(strike: number, optionType: 'CE' | 'PE', side: 'BUY' | 'SELL', row: ChainRow) {
    ...
    addBasketLeg({
      ...
      iv: isCe ? row.ceIv : row.peIv,
      delta: isCe ? row.ceDelta : row.peDelta,
      gamma: isCe ? row.ceGamma : row.peGamma,
      theta: isCe ? row.ceTheta : row.peTheta,
  ...
  function addEmptyOptLeg() {
    const atm = chain.spot
      ? chain.chainRows.reduce(
          (best, r) =>
            Math.abs(r.strike - chain.spot!) < Math.abs(best.strike - chain.spot!) ? r : best,
          chain.chainRows[0],
        )
      : chain.chainRows[Math.floor(chain.chainRows.length / 2)];
  ...
  function selectSymbol(inst: Record<string, unknown>) {
    const newSym = String(inst.asset || inst.stock_name || '')
      .replace(/\s+\d+$/, '')
      .replace(/\s+/g, '');
    if (!newSym) return;
  ...
  function addScripToBasket(inst: Record<string, unknown>) {
    ...
    const name = String(
      inst.zanskar_name || inst.nubra_name || inst.stock_name || inst.symbol || '',
    );
    const strike = inst.strike_price
      ? Number(inst.strike_price) > 10000
        ? Number(inst.strike_price) / 100
        : Number(inst.strike_price)
      : 0;
    ...
    addBasketLeg({
      symbol: asset,
      asset,
      exchange: String(inst.exchange || exch),
      optionType: optType,
      side: 'BUY',
      strike,
      expiry: exp,
  ...
  function applyTemplate(tmplId: string) {
    ...
    const atm = chain.spot
      ? strikes.reduce(
          (best, s) => (Math.abs(s - chain.spot!) < Math.abs(best - chain.spot!) ? s : best),
          strikes[0],
        )
    ...
    const newLegs: Leg[] = tmpl.legs.map((tl) => {
      ...
      const row = chain.chainRows.reduce((best, r) =>
  ...
  function loadSavedBasket(basket: {
    name?: string;
    legs: Array<Record<string, unknown>>;
    expiry: string;
  }) {
    setLegs(
      (basket.legs as unknown as Leg[]).map((l) => ({
        ...l,
        id: generateId(),
  ...
  const payoffData = useMemo(() => {
    ...
    const payoffLegs = legs.map((l) => ({
      ...
      type: l.optionType,
      side: l.side,
      qty: l.lots * l.lotSize,
      premium: l.ltp,
    }));
    const strikes = payoffLegs.map((l) => l.strike);
    const minS = Math.min(...strikes) * 0.9;
    const maxS = Math.max(...strikes) * 1.1;
  ...
  if (viewMode !== 'builder') {
    const filtered =
      sentimentFilter === 'All'
        ? STRATEGY_TEMPLATES
        : STRATEGY_TEMPLATES.filter((t) => t.sentiment === sentimentFilter);
          ...
          >
            <button
              ...
              style={{
                ...
                border: 'none',
                cursor: 'pointer',
                background: viewMode === 'prebuilt' ? '#5865f2' : 'transparent',
                color: viewMode === 'prebuilt' ? '#fff' : 'var(--text-secondary)',
              }}
            >
              Pre-built
            </button>
            <button
              ...
              style={{
                ...
                border: 'none',
                cursor: 'pointer',
                background: viewMode === 'saved' ? '#5865f2' : 'transparent',
                color: viewMode === 'saved' ? '#fff' : 'var(--text-secondary)',
              }}
            >
              Saved
            </button>
            ...
            <>
              ...
              >
                ...
                <span style={{ color: 'var(--green)', fontSize: 11 }}>
                  {chain.spot ? chain.spot.toLocaleString('en-IN') : ''}
                </span>
                ...
                {(['All', 'Bullish', 'Bearish', 'Neutral', 'Volatile'] as const).map((s) => (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      border:
                        sentimentFilter === s ? '1px solid #5865f2' : '1px solid var(--border)',
                ...
                return (
                  ...
                  >
                    <div style={{ position: 'relative', height: 100, padding: '8px 12px 0' }}>
                      <button
                        ...
                        style={{
                          ...
                          fontSize: 10,
                          fontWeight: 600,
                          background: '#5865f2' + '20',
                          color: '#5865f2',
                          border: '1px solid #5865f2' + '40',
                          cursor: 'pointer',
                        }}
                      >
                        View legs
                      ...
                      >
                        {(() => {
                          ...
                          const pts = mini
                            .map((p) => `${p.x},${28 - ((p.y - minY) / range) * 24}`)
                            .join(' ');
                    ...
                    >
                      ...
                      >
                        <span
                          style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)' }}
                        >
                          {tmpl.label}
                        </span>
              ...
              <div style={{ display: 'flex', height: '100%', gap: 24 }}>
                ...
                >
                  <svg viewBox="0 0 300 160" style={{ width: 300, height: 160, opacity: 0.6 }}>
                    ...
                    <path
                      d="M0,140 L60,140 L100,100 L150,60 L200,40 L250,30 L300,25"
                      fill="none"
                      stroke="#22c55e"
                      strokeWidth="2"
                    />
                    <path
                      d="M0,140 L60,140 L100,100 L150,60 L200,40 L250,30 L300,25 L300,160 L0,160 Z"
                      fill="url(#savedGreen)"
                    />
                    <path
                      d="M0,20 L50,30 L100,80 L150,100 L200,115 L250,120 L300,120"
                      fill="none"
                      stroke="#ef4444"
                      strokeWidth="1.5"
                      opacity={0.6}
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                    Build and save strategies to analyze your trades better
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  >
                    Take control and build one that works for you.
                  </div>
                  <button
                ...
                {persistence.savedBaskets.map((b) => (
                  ...
                  >
                    ...
                    >
                      {editingBasketId === b.basket_id ? (
                        <input
                          ...
                          onBlur={() => {
                            if (editingBasketName.trim())
                              persistence.updateBasketName(b.basket_id, editingBasketName);
                          ...
                          onKeyDown={(e) => {
                            ...
                            if (e.key === 'Escape') setEditingBasketId(null);
                          }}
                          style={{
                            fontWeight: 700,
                            fontSize: 14,
                        ...
                        <span
                          ...
                          onDoubleClick={() => {
                            setEditingBasketId(b.basket_id);
                            setEditingBasketName(b.name);
                          }}
                      ...
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        ...
                        <button
                          onClick={() => persistence.deleteSavedBasket(b.basket_id)}
                          style={{
                            background: 'none',
                            border: 'none',
                            color: 'var(--text-secondary)',
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginBottom: 10 }}>
                      {b.symbol} · {formatExpiry(b.expiry)} · {b.legs.length} legs
                    </div>
                    <button
                      onClick={() => loadSavedBasket(b)}
                      style={{
                        width: '100%',
                        padding: '7px 0',
                        borderRadius: 8,
        ...
        >
          <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
            ...
            >
              <span
                ...
                onClick={() => setShowSymSearch(!showSymSearch)}
              >
                🔍
              </span>
              {showSymSearch ? (
                      ...
                      {symResults.map((inst, i) => (
                        ...
                        >
                          ...
                          <span style={{ color: 'var(--text-muted)', marginLeft: 6, fontSize: 10 }}>
                            ...
                            {String(inst.derivative_type || inst.asset_type || '')}
                          </span>
                        </button>
                      ))}
                    </div>
                ...
                <span
                  ...
                  onClick={() => setShowSymSearch(true)}
                >
                  {sym}
                </span>
              )}
              <span style={{ fontSize: 13, color: 'var(--green)' }}>
                {chain.spot ? chain.spot.toLocaleString('en-IN', { minimumFractionDigits: 2 }) : ''}
              </span>
            ...
            {chain.spot && (
              <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
                ...
                <span style={{ color: totalPrice >= 0 ? 'var(--green)' : 'var(--red)' }}>
                  +0.00 (0.00%)
                </span>
          ...
          <div style={{ padding: '0 12px 8px' }}>
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: '30px 88px 96px 30px 68px 52px 64px 24px',
                alignItems: 'center',
                gap: 2,
                padding: '8px 0',
                borderBottom: '1px solid var(--border)',
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 700,
                    fontSize: 11,
                    background: leg.side === 'BUY' ? 'var(--green-dim)' : 'var(--red-dim)',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <button
                    onClick={() => {
                      const idx = chain.chainRows.findIndex((r) => r.strike === leg.strike);
                      if (idx > 0) {
                        const r = chain.chainRows[idx - 1];
                        const newLtp = leg.optionType === 'CE' ? r.ceLtp : r.peLtp;
                        updateLeg(leg.id, {
                          strike: r.strike,
                          ltp: newLtp,
                          entryLtp: newLtp,
                          refId: leg.optionType === 'CE' ? r.ceRefId : r.peRefId,
                          nubraName: leg.optionType === 'CE' ? r.ceNubraName : r.peNubraName,
                        });
                      }
                    }}
                    style={{
                  ...
                  <button
                    onClick={() => {
                      const idx = chain.chainRows.findIndex((r) => r.strike === leg.strike);
                      if (idx < chain.chainRows.length - 1) {
                        const r = chain.chainRows[idx + 1];
                        const newLtp = leg.optionType === 'CE' ? r.ceLtp : r.peLtp;
                        updateLeg(leg.id, {
                          strike: r.strike,
                ...
                <button
                  onClick={() =>
                    updateLeg(leg.id, { optionType: leg.optionType === 'CE' ? 'PE' : 'CE' })
                  }
                  style={{
                    width: 26,
                    height: 22,
                    ...
                    fontWeight: 700,
                    fontSize: 10,
                    background: leg.optionType === 'CE' ? 'var(--green-dim)' : 'var(--red-dim)',
                    color: leg.optionType === 'CE' ? 'var(--green)' : 'var(--red)',
                  }}
                >
                  {leg.optionType}
                </button>
            ...
            <div style={{ display: 'flex', gap: 12, padding: '10px 0', fontSize: 12 }}>
              <button
                onClick={addEmptyOptLeg}
                disabled={!chain.chainRows.length}
                style={{
                  background: 'none',
                  border: 'none',
                  color: '#5865f2',
                  ...
                  fontWeight: 600,
                  fontSize: 12,
                  opacity: chain.chainRows.length ? 1 : 0.4,
                }}
              >
                + Add OPT Leg
              </button>
              ...
              <div>
                ...
                <span
                  style={{
                    fontSize: 13,
                    fontWeight: 700,
                    color: totalPremium >= 0 ? 'var(--green)' : 'var(--red)',
                  }}
                >
            ...
            {totalPremium > 0 && (
              ...
              >
                <span style={{ color: 'var(--text-muted)' }}>ℹ</span> Negative prices indicate that
                executing this strategy will result in a net cash inflow.
              </div>
            )}
                ...
                {margin ? (
                  ...
                  <div
                    style={{
                      padding: '12px 14px',
                      fontSize: 11,
                      color: marginError ? 'var(--red)' : 'var(--text-muted)',
                    }}
                  >
            ...
            >
              ...
              <div style={{ padding: '0 14px' }}>
                <div
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '32px 90px 60px 40px 60px 60px 60px 60px',
                    alignItems: 'center',
                    padding: '8px 0',
                    borderBottom: '1px solid var(--bg-card)',
                    fontSize: 10,
                ...
                {legs.map((leg) => (
                  ...
                  >
                    ...
                    <span style={{ color: 'var(--text-primary)', fontSize: 11 }}>
                      {formatExpiry(leg.expiry)} {leg.optionType}
                    </span>
                ...
                >
                  ...
                  <span>{greeks.gamma.toFixed(4)}</span>
            ...
            >
              ...
              <div style={{ padding: '0 14px' }}>
                <div
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '32px 90px 80px 50px 1fr',
                    alignItems: 'center',
                    padding: '8px 0',
                    borderBottom: '1px solid var(--bg-card)',
                    fontSize: 10,
                ...
                >
                  ...
                  <span>Strike</span>
                  <span>Qty</span>
                  <span style={{ textAlign: 'right' as const }}>Unrealized P&L</span>
                </div>
                {legs.map((leg) => {
                  const mtm = legMtm(leg);
                  return (
                ...
                >
                  ...
                  >
                    {totalMtm >= 0 ? '+' : '-'}₹{fmtPrice(Math.abs(totalMtm))}
                  </span>
                </div>
                <div style={{ padding: '6px 0 8px', fontSize: 10, color: 'var(--text-muted)' }}>
          ...
          onMouseDown={(e) => {
            ...
            const onMove = (ev: MouseEvent) => {
              ...
              setLeftWidth(
                Math.max(
                  320,
                  Math.min(800, resizeRef.current.startW + (ev.clientX - resizeRef.current.startX)),
                ),
              );
            };
            const onUp = () => {
            ...
            <div>
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>
                {breakevenPoints.length
                  ? breakevenPoints.map((bp) => bp.toLocaleString('en-IN')).join(', ')
                  : '—'}
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>
                {breakevenPoints.length && chain.spot
                  ? (() => {
                      const bullish = payoffData.filter(
                        (d) => d.spot >= chain.spot! && d.pnl > 0,
                      ).length;
            ...
            {(
              ...
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  border: 'none',
                  borderBottom: rightTab === key ? '2px solid #5865f2' : '2px solid transparent',
                  background: 'transparent',
            ...
            <>
              <div style={{ flex: 1, minHeight: 250, padding: '0 8px' }}>
                <SvgChart
                      ...
                      ? [
                          {
                            ...
                            color: '#5865f2',
                            dashed: true,
                            label: chain.spot.toLocaleString('en-IN'),
                            labelColor: '#5865f2',
                          },
                        ]
                      : []),
                  ...
                  yFormatter={(v) =>
                    `Rs ${Math.abs(v) >= 1000 ? (v / 1000).toFixed(1) + 'K' : String(v)}`
                  }
                  tooltipFormatter={(d) =>
              ...
              >
                <span>
                  ...
                  <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>
                    {targetDays} Day from Expiry
                  </span>
                </span>
                <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <button
                    onClick={() => setTargetDays(Math.max(0, targetDays - 1))}
                    style={{
                      width: 22,
                      height: 22,
                      borderRadius: 4,
                  ...
                  <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>
                    {chain.expiry ? formatExpiry(chain.expiry) : '—'}
                  </span>
                  <button
                    onClick={() => setTargetDays(targetDays + 1)}
                    style={{
                      width: 22,
                      height: 22,
                      borderRadius: 4,
        ...
        >
          ...
          >
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  persistence.saveBasket(saveName, sym, chain.expiry, legs).then((r) => {
                    setPlaced(r);
        ...
        >
          ...
          >
            ...
            <input
              type="text"
              value={addScripQuery}
              onChange={(e) => searchScrip(e.target.value)}
              autoFocus
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
                      ...
                      {inst.strike_price
                        ? ` · Strike: ${Number(inst.strike_price) > 10000 ? (Number(inst.strike_price) / 100).toLocaleString('en-IN') : Number(inst.strike_price).toLocaleString('en-IN')}`
                        : ''}
                      {inst.expiry ? ` · ${formatExpiry(String(inst.expiry))}` : ''}
                      {inst.lot_size ? ` · Lot: ${inst.lot_size}` : ''}
                    </div>
                  </div>
                  <span style={{ color: '#5865f2', fontWeight: 600, fontSize: 11, flexShrink: 0 }}>
                    + Add
                  </span>
                </button>
              ))}
      ...
      {placed && (
        <div
          style={{
            ...
            background: placed.ok ? 'var(--green-dim)' : 'var(--red-dim)',
            color: placed.ok ? 'var(--green)' : 'var(--red)',
            border: `1px solid ${placed.ok ? '#22c55e40' : '#ef444440'}`,
          }}
        >
          {placed.msg}
        </div>
