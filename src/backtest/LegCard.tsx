...
export default function LegCard({ leg, index, onChange, onRemove }: Props) {
  ...
  return (
    <div
      className={`rounded-lg border p-2 flex flex-col gap-2 ${leg.enabled ? 'border-[var(--border)] bg-[var(--bg-secondary)]' : 'border-[var(--border)] bg-[var(--bg-card)] opacity-55'}`}
    >
      {/* header row */}
      <div className="flex items-center gap-1.5">
        ...
        <button
          onClick={() => set({ enabled: !leg.enabled })}
      ...
      <div className="grid grid-cols-2 gap-1.5">
        ...
        <label>
          ...
          <select
            value={leg.strike.method}
            onChange={(e) =>
              set({ strike: { ...leg.strike, method: e.target.value as StrikeMethod } })
            }
            className={inputCls}
          >
            {STRIKE_METHODS.map((m) => (
          ...
          {RANGE_STRIKE(leg.strike.method) ? (
            <div className="flex gap-1">
              <input
                ...
                aria-label="min"
                placeholder="min"
                step={strikeStep(leg.strike.method)}
                value={strikeRangeVal(leg, 'lo')}
                onChange={(e) =>
                  set({
                    strike: {
                      ...leg.strike,
                      ...strikeRangePatch(leg.strike.method, 'lo', Number(e.target.value)),
                    },
                  })
                }
                className={inputCls}
              ...
              <input
                ...
                onChange={(e) =>
                  set({
                    strike: {
                      ...leg.strike,
                      ...strikeRangePatch(leg.strike.method, 'hi', Number(e.target.value)),
                    },
                  })
                }
                className={inputCls}
            ...
            <input
              type="number"
              value={strikeParamValue(leg)}
              step={strikeStep(leg.strike.method)}
              onChange={(e) =>
                set({
                  strike: {
                    ...leg.strike,
                    ...strikeParamPatch(leg.strike.method, Number(e.target.value)),
                  },
                })
              }
              className={inputCls}
        ...
        <div className="flex gap-1">
          <label className="flex-1">
            ...
            <select
              ...
              onChange={(e) =>
                set({ stopLoss: { ...leg.stopLoss, type: e.target.value as SLTargetType } })
              }
              className={inputCls}
            >
              {SL_TYPES.map((s) => (
          ...
          {leg.stopLoss.type !== 'NONE' && (
            <label className="w-16">
              ...
              <input
                ...
                onChange={(e) =>
                  set({ stopLoss: { ...leg.stopLoss, value: Number(e.target.value) } })
                }
                className={inputCls}
              />
            </label>
        ...
        <div className="flex gap-1">
          <label className="flex-1">
            ...
            <select
              ...
              onChange={(e) =>
                set({ target: { ...leg.target, type: e.target.value as SLTargetType } })
              }
              className={inputCls}
            >
              {SL_TYPES.map((s) => (
          ...
          {trail.type !== 'NONE' && (
            <div className="grid grid-cols-3 gap-1">
              <label>
                ...
                <input
                  type="number"
                  value={trail.trigger ?? 0}
                  onChange={(e) => setTrail({ trigger: Number(e.target.value) })}
                  className={inputCls}
                />
              </label>
              {(trail.type === 'LOCK' || trail.type === 'LOCK_AND_TRAIL') && (
                <label>
                  ...
                  <input
                    type="number"
                    value={trail.lock ?? 0}
                    onChange={(e) => setTrail({ lock: Number(e.target.value) })}
                    className={inputCls}
                  />
                </label>
              )}
                ...
                <>
                  <label>
                    ...
                    <input
                      type="number"
                      value={trail.step ?? 0}
                      onChange={(e) => setTrail({ step: Number(e.target.value) })}
                      className={inputCls}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      type="number"
                      value={trail.trail ?? 0}
                      onChange={(e) => setTrail({ trail: Number(e.target.value) })}
                      className={inputCls}
                    />
                  </label>
                </>
            ...
            <div className="flex items-center gap-2">
              <label className="w-20">
                ...
                <input
                  ...
                  min={0}
                  value={reentry.max ?? 0}
                  onChange={(e) => setReentry({ max: Math.max(0, Number(e.target.value) || 0) })}
                  className={inputCls}
                />
              </label>
              <label className="flex items-center gap-1 mt-3 text-[10px] text-[var(--text-secondary)]">
                <input
                  type="checkbox"
                  checked={reentry.onStopLoss ?? true}
              ...
              <label className="flex items-center gap-1 mt-3 text-[10px] text-[var(--text-secondary)]">
                <input
                  type="checkbox"
                  checked={reentry.onTarget ?? false}
...
function strikeParamLabel(m: StrikeMethod): string {
  switch (m) {
    ...
    case 'STRADDLE_WIDTH':
      return 'Straddle width ×';
    case 'ATM_STRADDLE_PREMIUM_PCT':
      return '% of ATM straddle';
...
function strikeStep(m: StrikeMethod): number {
  switch (m) {
    ...
    case 'ATM_STRADDLE_PREMIUM_PCT':
      return 1;
    default:
      return 5; // premium-based criteria
  }
}
function strikeParamValue(leg: Leg): number {
  ...
  switch (s.method) {
    case 'ATM':
      return s.atmOffset ?? 0;
    case 'CLOSEST_PREMIUM':
    case 'PREMIUM_GTE':
    case 'PREMIUM_LTE':
      return s.premiumTarget ?? 0;
    case 'STRADDLE_WIDTH':
      return s.straddleWidthMult ?? 1;
    case 'ATM_STRADDLE_PREMIUM_PCT':
      return s.straddlePremiumPct ?? 0;
    case 'POINTS_FROM_SPOT':
      return s.pointsFromSpot ?? 0;
    case 'PERCENT_FROM_SPOT':
      return s.percentFromSpot ?? 0;
    case 'FIXED_STRIKE':
      return s.absoluteStrike ?? 0;
    case 'DELTA':
      return s.targetDelta ?? 0.3;
    case 'PREMIUM_RANGE':
      return s.premiumMin ?? 0; // unused (range UI), kept exhaustive
    case 'DELTA_RANGE':
      return s.deltaMin ?? 0; // unused (range UI), kept exhaustive
  }
}
function strikeParamPatch(m: StrikeMethod, v: number): Partial<Leg['strike']> {
  switch (m) {
    case 'ATM':
      return { atmOffset: Math.round(v) };
    case 'CLOSEST_PREMIUM':
    case 'PREMIUM_GTE':
    case 'PREMIUM_LTE':
      return { premiumTarget: v };
    case 'STRADDLE_WIDTH':
      return { straddleWidthMult: v };
    case 'ATM_STRADDLE_PREMIUM_PCT':
      return { straddlePremiumPct: v };
    case 'POINTS_FROM_SPOT':
      return { pointsFromSpot: v };
    case 'PERCENT_FROM_SPOT':
      return { percentFromSpot: v };
    case 'FIXED_STRIKE':
      return { absoluteStrike: v };
    case 'DELTA':
      return { targetDelta: v };
    case 'PREMIUM_RANGE':
      return { premiumMin: v }; // unused (range UI), kept exhaustive
    case 'DELTA_RANGE':
      return { deltaMin: v }; // unused (range UI), kept exhaustive
  }
}
