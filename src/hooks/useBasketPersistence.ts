...
export function useBasketPersistence(): BasketPersistenceApi {
  ...
  async function loadSavedBaskets() {
    try {
      ...
      console.warn('[Basket] loadSavedBaskets failed:', e);
    }
  }
  ...
  async function deleteSavedBasket(id: string) {
    try {
      await fetch(`/paper/baskets/${id}`, { method: 'DELETE' });
      loadSavedBaskets();
    } catch (e) {
      console.warn('[Basket] delete failed:', e);
    }
  }

  async function updateBasketName(id: string, name: string) {
    try {
      await fetch(`/paper/baskets/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.warn('[Basket] updateBasketName failed:', e);
    }
  }

  async function renameStrategy(basketGroupId: string, name: string): Promise<boolean> {
    try {
      const res = await fetch('/paper/strategy/rename', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
