...
export function useMarginCalc(
  ...
  exch: string,
  multiplier: number,
  onLegsResolved?: (resolved: any[]) => void,
): MarginCalcApi {
  ...
  const fetchMargin = useCallback(async () => {
    ...
    try {
      ...
      const benefit =
        directBenefit > 0
          ? directBenefit
          : individualSum > total && total > 0
            ? individualSum - total
            : 0;
      setMargin({
        total,
        benefit: benefit > 0 ? benefit : undefined,
        span: span > 0 ? span : undefined,
        exposure: exposure > 0 ? exposure : undefined,
