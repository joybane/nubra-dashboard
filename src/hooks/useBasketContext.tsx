...
export function BasketProvider({ children }: { children: React.ReactNode }) {
      ...
      setLegs((prev) => {
        ...
        const next = prev.map((leg) => {
          ...
          if (
            merged.ltp !== leg.ltp ||
            merged.iv !== leg.iv ||
            merged.delta !== leg.delta ||
            merged.gamma !== leg.gamma ||
            merged.theta !== leg.theta ||
            merged.vega !== leg.vega
          ) {
            changed = true;
            return merged;
          }
    ...
    setLegs((prev) => {
      ...
      const next = prev.map((leg) => {
        ...
        if (
          ...
          (found.delta as number | undefined) === leg.delta
        )
          return leg;
        changed = true;
        return {
  ...
  // ── Order placement ────────────────────────────────────────────────────────
  const fetchMarginRequiredPaise = useCallback(
    async (orderLegs: BasketLeg[]): Promise<number | undefined> => {
      ...
      const res = await fetch('/paper/margin/basket', {
        ...
        body: JSON.stringify({
          ...
          orders: validLegs.map((l) => ({
            ...
            order_delivery_type:
              l.deliveryType === 'IDAY' ? 'ORDER_DELIVERY_TYPE_IDAY' : 'ORDER_DELIVERY_TYPE_CNC',
          })),
        }),
      });
  ...
  const placeBasket = useCallback(
    async (opts?: PlaceBasketOptions): Promise<{ ok: boolean; msg: string }> => {
      ...
      try {
        ...
        if (!marginRequired)
          throw new Error(
            'Margin unavailable. Please wait for the margin calculation and try again.',
          );
        const res = await fetch('/paper/orders/basket', {
          ...
          body: JSON.stringify({
            ...
            orders: sorted.map((l) => ({
              ...
              order_type: ORDER_TYPE_MAP[l.orderType],
              order_side: l.side === 'BUY' ? 'ORDER_SIDE_BUY' : 'ORDER_SIDE_SELL',
              order_qty: l.lots * l.lotSize,
        ...
        return {
          ok: true,
          msg: `${d.orders?.length ?? legs.length} order(s) placed & saved as "${finalName}"`,
        };
      } catch (e) {
