...
| `orders[].stratTags` | array | Optional | Client-defined strategy tag for grouping and traceability. Pass exactly one tag inside the list. Tag names must use hyphen-separated text only, such as `abc-def`. Do not use underscores, spaces, colons, plus signs, timestamps, or other special characters |
...
| `stratTags` | Client-defined strategy tag echoed back for downstream grouping and retrieval. Tag values use hyphen-separated text only. |
...
!!! important "stratTags"
    Each order item can pass exactly one tag inside `stratTags`. Use hyphen-separated tag names only, such as `rest-api-v3-multi-order-buy`. Do not use underscores, spaces, colons, plus signs, timestamps, or other special characters.
...
| `intentOrderResponse.stratTags` | `string[]` | Strategy tags. Tag values use hyphen-separated text only. |
